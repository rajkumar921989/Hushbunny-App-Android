...
import com.hushbunny.app.ui.service.OnBoardingService
...
sealed class LoginResponseStatus {
    ...
    data class Success(val loginResponse: UserData?) : LoginResponseStatus()
}
...
class OnBoardingRepositoryImpl @Inject constructor(private val onBoardingService: OnBoardingService) :
    ...
    override suspend fun userLogin(loginRequest: LoginRequest): LoginResponseStatus {
        ...
        return when (response.statusCode) {
            APIConstants.API_RESPONSE_200 -> LoginResponseStatus.Success(response.data)
    ...
    override suspend fun changePassword(changePasswordRequest: ChangePasswordRequest): PasswordResponse {
        return onBoardingService.changePassword(changePasswordRequest)
    }
}